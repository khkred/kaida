...
GoRouter createRouter(WidgetRef ref) {
  ...
  return GoRouter(
      ...
      redirect: (context, state) async{
        ...
        final isLoggedIn = authState is AuthStateUnAuthenticated;
        final isLoggingIn = state.uri.path == Routes.signIn || state.uri.path == Routes.signUp;

        // Onboarding check
        final isOnBoardingComplete = await SharedPreferencesUtil.isOnboardingComplete();
        ...
        if( isLoggedIn && (state.uri.path == Routes.signIn || state.uri.path == Routes.signUp || state.uri.path == Routes.onboarding)) {
